...
function BallPhotoCard({ photo }: { photo: { label: string; src: string; alt: string } }) {
  return (
    <figure className="overflow-hidden rounded-[1.35rem] border border-parade-gold/30 bg-gradient-to-br from-parade-purpleDeep via-parade-purpleDark to-parade-purple p-2 text-white shadow-card">
      <div className="overflow-hidden rounded-[1rem] bg-black/30">
        <img
          src={photo.src}
          alt={photo.alt}
          loading="lazy"
      ...
      <figcaption className="px-3 py-4">
        <p className="text-xs font-black uppercase tracking-[0.18em] text-parade-goldBright">75th Annual Ball</p>
        <p className="mt-1 text-xl font-black text-white">{photo.label}</p>
      </figcaption>
    </figure>
  );
}
